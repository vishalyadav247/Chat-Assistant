...
// Admin → Coupons (spec 15 · discount codes). Operator creates codes here and
...
const ADD_MODAL = "admin-add-promo";
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (intent === "feature") {
    // Master switch. validatePromoCode() re-reads this on every attempt, so a
    // merchant holding a code cannot redeem it once this is off.
    await saveRuntimeConfig({ promoCodesEnabled: form.get("enabled") === "true" });
...
const SUCCESS_MESSAGE: Record<string, string> = {
  feature: "Coupon availability updated",
  add: "Coupon created",
...
export default function AdminPromoCodes() {
  ...
  return (
    <AdminShell adminEmail={data.adminEmail}>
      <AdminPage
        heading="Coupons"
        subheading="Discount codes for paid plans. Share a code with a merchant by email or chat; they apply it on Plan & Usage and Shopify applies the discount to their subscription — visible on the approval page and on every invoice."
      >
        <s-stack gap="base">
          {/* Master switch first: whether the feature exists at all is a bigger
          ...
          <AdminCard heading="Availability">
            <s-switch
              label="Discount codes are available to merchants"
              details={
                data.couponsEnabled
                  ? "Merchants see the “Have a code?” field on Plan & Usage and can redeem the codes below."
                  : "The field is hidden on Plan & Usage and every redemption is refused server-side — existing codes stay here, unused, until you switch this back on. Subscriptions already discounted keep their discount."
              }
              checked={data.couponsEnabled}
              onInput={(e) =>
                fetcher.submit(
                  { intent: "feature", enabled: String(e.currentTarget.checked) },
                  { method: "post" },
                )
              }
            />
