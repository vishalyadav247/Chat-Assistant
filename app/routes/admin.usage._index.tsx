...
import { normalizeRange, usageOverview } from "../lib/admin/usage-report.server";
...
// Admin → Usage (spec 19). Token consumption + estimated cost per merchant.
// Cross-tenant aggregate BY DESIGN; guarded by requireAdminUser.
...
export default function AdminUsage() {
  ...
  return (
    <AdminShell adminEmail={data.adminEmail}>
      <AdminPage heading="Usage" subheading="Token consumption and estimated OpenAI cost per merchant. Counts come straight from the API — every chat, summary, moderation and embedding call is attributed to the store that triggered it.">
        <s-stack gap="base">
          <TabPills
          ...
          {data.grand.unpricedModels.length > 0 ? (
            <s-banner tone="warning">
              ...
              cost. Add the model to <code>app/lib/admin/llm-pricing.ts</code>.
            </s-banner>
          ) : null}

          <AdminCard heading="By merchant">
            {active.length === 0 ? (
              <EmptyState
                icon="chart-line"
                    ...
                    <s-table-row key={shop.shopId}>
                      <s-table-cell>
                        <s-stack gap="small-300">
                          <Link to={`/admin/usage/${shop.shopId}?days=${data.days}`}>
          ...
          <AdminCard heading="Where the tokens go">
