...
export const RUNTIME_SECRET_KEY = "admin:runtime";
...
export async function loadRuntimeConfig(): Promise<void> {
  try {
    ...
    if (!row) {
      ...
      else logError("admin_runtime_config_invalid", parsed.error.issues[0]);
    }
  } catch (error) {
    logError("admin_runtime_config_load_error", error);
  }
...
export function runtimeConfig(): EffectiveRuntime {
  ...
  return {
    ...
    promoCodesEnabled: stored.promoCodesEnabled ?? envBool("PROMO_CODES_ENABLED") ?? true,
  };
}
...
export async function saveRuntimeConfig(patch: Partial<RuntimeConfig>): Promise<void> {
  // Re-read before merging. Each section of /admin/settings saves a PATCH
