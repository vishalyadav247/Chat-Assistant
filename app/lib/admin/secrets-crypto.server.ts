...
function secretKey(): Buffer {
  if (!cachedKey) {
    ...
    if (!process.env.SHOPIFY_API_SECRET) {
      logWarn("admin_secret_key_fallback", "SHOPIFY_API_SECRET absent — deriving settings key from DATABASE_URL");
    }
    cachedKey = scryptSync(material, "chatconvert:admin:secrets:v1", 32);
  }
  return cachedKey;
}
...
export function decryptSecret(stored: string): string {
  ...
  try {
    ...
    logError("admin_secret_decrypt_failed", error instanceof Error ? error.message : error);
    return "";
  }
}
