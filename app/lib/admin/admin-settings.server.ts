...
export const AI_SECRET_KEY = "admin:ai";
  ...
  if (!loading) {
    loading = (async () => {
      try {
        ...
        if (parsed && !parsed.success) logError("admin_ai_config_invalid", parsed.error.issues[0]);
      } catch (error) {
        logError("admin_ai_config_load_error", error);
      }
      cachedAt = Date.now();
      return cached;
    })().finally(() => {
...
export const EMBEDDING_MODEL_KEY = "admin:embedding-model";
...
export async function getStoredPlanConfig(): Promise<PlanConfig> {
  ...
  // A corrupt row used to be swallowed into `{}`. The admin UI then merged
