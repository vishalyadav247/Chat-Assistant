...
2. Load and follow the project skills relevant to the surfaces you touch: `spec-workflow`, `db-tenancy` (any DB work), `shopify-app-dev` (routes/webhooks/API), `polaris-admin-ui` (admin pages — open the referenced design HTML in `.claude/resources/html_design/` (local-only, not in git — on a fresh clone the shipped page is the reference)), `theme-extension-widget` (storefront), `ai-pipeline` (LLM/search).
