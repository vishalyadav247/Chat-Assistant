...
- **FAQ block gate (2026-09-16, owner):** the block (search + list) renders only when the shop has at least one FEATURED published FAQ (`faqAvailable` = featured count > 0), whatever the Chatbox switch says. Published-but-none-featured used to keep an empty search box (QA D14) that looked broken; the Chatbox preview already used this rule.
- **Branding link (2026-09-16, owner):** "Powered by ChatConvert" links "ChatConvert" to the App Store listing (`brandingUrl` in widget config = `https://apps.shopify.com/<effective handle>` — Admin → Settings, env, then `DEFAULT_APP_STORE_HANDLE`), new tab, `rel=noopener`. The renderer only links an apps.shopify.com URL.
