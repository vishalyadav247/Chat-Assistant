...
| `file` | .pdf .txt .json ≤2MB, **.csv up to the plan's `csv_upload_mb`** (2026-09-16, user decision: 1/2/5/10 MB, clamped to a 10MB server ceiling — base64 form post under nginx's 25M, and the ingest must finish inside the 15-min job expiry; 0 = no CSV). CSV: first row = headers, each row becomes a "Header: value" record separated by blank lines so chunks split between rows; a header-only CSV is rejected without using quota; a CSV counts against `file_uploads`. Q&A CSVs still belong in the FAQs importer. Chunks are inserted 200 per statement (was one create + one UPDATE per chunk); **merchant-written title required** (2026-09-11) — shown in Manage sources and used as the chunk topic; filename kept in metadata (its extension decides parsing); each file is its own row and deletable | files: 5 on every plan |
| `table` | **Lookup table** (spec 28, 2026-09-16) — a CSV stored as rows (`lookup_rows`) that the agent filters exactly with `lookup_table`; the upload modal offers it next to reference-text CSV. Counts as a file upload; rows capped by `lookup_rows` | `lookup_rows` 1k / 10k / 50k / 200k |
